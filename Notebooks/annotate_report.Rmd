...
names(consensus) = basename(dirname(consensus))
...
if(cons_tools == 'all'){
  cons_tools = c('Consensus', tools)
}else{
  cons_tools = c('Consensus', cons_tools)
}
```
...
for(r in refs){
  list[[r]]$lab = data.table::fread(paste0(params$output_dir, '/model/', r, '/labels.csv'), header = T, fill = T)
 
  list[[r]]$pred = data.table::fread(consensus[r]) %>% 
...
for(r in refs){
  ...
  calculate_percentage_unresolved(list[[r]]$pred, order = tools, cons_tools = cons_tools) %>% 
  ...
  for(t in tools){
    ...
    if(t == 'Consensus' & params$cons_type %in% c('CAWPE_T', 'CAWPE_CT')){

        cat("<h3>CAWPE Score</h3>")
        cat("\n")

        l = feature_plot_seurat_meta(query, 'CAWPE')
